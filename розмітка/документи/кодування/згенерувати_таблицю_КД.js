...
fs.writeFileSync('відповідні_символи_юнікоду.ю8', charDefinitions.map(([char, definition], index) => {
    ...
    const name = definition.replaceAll(' ', '_').replaceAll('\"', '');
    const lines = [];
    lines.push(`синонім ${name} = ${char.charCodeAt(0)};`);
    return lines.join('\n');
}).join('\n'));
...
fs.writeFileSync('отримати_символ_ю8_з_т32.ю8', charDefinitions.map(([char, definition], index) => {
    ...
    if (encoded.length === 2) {
        lines.push(`  вихід_першого_байта::вміст = п8(192п32 | (п32(відповідні_символи_юнікоду::${name}) >> 6));`);
        lines.push(`  вихід_другого_байта::вміст = п8(128п32 | (п32(відповідні_символи_юнікоду::${name}) & 63));`);
        lines.push(`  вихід_розміру::вміст = 2;`);
        lines.push(`  вернути так;`);
    }
    if (encoded.length === 3) {
        lines.push(`  вихід_першого_байта::вміст = п8(224п32 | (п32(відповідні_символи_юнікоду::${name}) >> 12));`);
        lines.push(`  вихід_другого_байта::вміст = п8(128п32 | ((п32(відповідні_символи_юнікоду::${name}) >> 6) & 63));`);
        lines.push(`  вихід_третього_байта::вміст = п8(128п32 | (п32(відповідні_символи_юнікоду::${name}) & 63));`);
...
мовою.
**КД** підтримує загальновживані символи з ASCII.
